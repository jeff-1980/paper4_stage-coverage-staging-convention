...
no retraining). See `legacy_leaked/` for why the *first* version's numbers
were withdrawn, and "Known retracted analyses" below for two more recent,
narrower retractions that are specific to this v3 round.

## v3 changelog (Stage 0-R2)

Three implementation fixes, applied together in one canonical rebuild
(`per_sample_final_v2.csv`, replacing v1's `per_sample_final.csv`). None of
them retrain the base models — only inference-time and analysis-time code
changed.
...
3. **Mondrian CP (split) redefinition, replacing "frozen"**
   ...
   quantile per stage; `run_mondrian_split()` applies that one fixed
   per-stage quantile to every evaluation sample — no `.update()`, no shared
   mutable state, and evaluation-unit order is provably irrelevant (the
   function carries no state at all). Column names (`mf`, `covered_mf`,
   `width_mf`, `is_inf_mf`, ...) are unchanged from v1; what they mean is not.
...
Two further v3 analyses (not comparisons to v1, new in this round):
...
- **Cohort restriction** (instances/engines with ≥1 window in all 3 stages
  ...
  (`tables/table_trainpct_full_vs_cohort.{csv,tex}`,
  `tables/table_cusum_cohort_check.csv`).
- **Paired-difference CIs** via a unique-engine-block bootstrap (10,000
  ...
  difference of all 16 cells, on the smallest-N subset (`tables/paired_diff_v2.csv`).

## Directory overview

- `data/per_sample_final_v2.csv` — the canonical per-sample data source for
  this (third) version. One row per (fd, seed, engine, t): raw model outputs
  (mu, sigma, y_true, s_t), all four methods' interval bounds (MC-Dropout,
  Split CP, Mondrian CP online, Mondrian CP split — Mondrian's at
  h ∈ {5, 7, 10}, k=0.7 fixed), and four staging-convention labels
  (fixed-fraction, CUSUM, train-set-percentile, occupancy-matched).
  `data/per_sample_final_v2.md5` is its md5
  (`d223342260e8b146b5cfba78f53b422d`). Every table and figure under `tables/`
  and `figures/` with a `_v2` suffix is generated from this one file.
  `data/per_sample_final.csv` (+ `.md5`) is the **v1** file, kept on disk,
  **superseded** — it predates all three fixes in the changelog above:
  (1) it uses the old, variance-understating MC-Dropout formula; (2) its
  `stage_cusum` labels use the old fixed 10-sample baseline window and the
  old `cp=n-1` untriggered convention; (3) its Mondrian "frozen" columns are
  the old adaptive-deepcopy definition, not the new non-adaptive split
  definition. Do not mix v1 and v2 columns in one table.
- `data/per_unit_clocks_final_v2.csv`, `data/trainpct_thresholds_final_v2.csv`
  — secondary per-unit and per-(fd,seed) statistics produced alongside the
  v2 canonical file (RUL-clip knee position, CUSUM change-point position
  under the new definition, the train-set 33rd/67th raw-cycle percentiles
  used by the train-set-percentile convention). `per_unit_clocks_final.csv`
  and `trainpct_thresholds_final.csv` (no suffix) are the superseded v1
  counterparts, kept on disk.
- `models/` — 20 trained Bayesian LSTM checkpoints (`best_{FD}_seed{N}.pt`,
  ...
  scaler fit statistics and their source). **Unchanged since v1/v2** — none
  of the Stage 0-R2 fixes retrain anything.
- `scripts/` — `train_clean.py` (trains the 20 checkpoints), `inference_clean.py`
  ...
  unchanged this round; still contains an unused `run_mondrian_frozen`
  function left over from before the split redefinition, kept rather than
  edited since this file itself required no logic changes in Stage 0-R2),
  `build_canonical.py` (produces `data/per_sample_final_v2.csv`),
  `aggregate.py` (produces everything in `tables/` from that one file),
  `figures_final.py` (produces everything in `figures/`). Also includes the
  two small local packages these scripts import: `scripts/models/bayesian_lstm.py`
  and `scripts/conformal/{mondrian_cp,adaptive_lambda_cp}.py`.
- `tables/` — every current `.csv`/`.tex` table, all generated by
  `aggregate.py` from `data/per_sample_final_v2.csv`: `table_main_v2`
  (main ECR table, 4 conventions × 4 methods × 3 stages), `table_mondrian_unbounded_v2`
  (unbounded-interval rates), `table_B_trainpct_v2` / `table_trainpct_cohort` /
  `table_trainpct_full_vs_cohort` (train-set-percentile convention: full
  availability vs. the ≥1-window-per-stage cohort, side by side),
  `table_cusum_cohort_check` (same cohort restriction applied to CUSUM, as a
  confirmation check), `paired_diff_v2` (within-engine paired differences,
  both bootstrap variants), `table_matched_v2` (occupancy-matched convention),
  `bucket_geometry_v2`, `A1_clocks_v2`, `A2_decile_v2` (mechanism
  diagnostics). Every `.tex` fragment's header comment records the
  `aggregate.py` function that produced it and `per_sample_final_v2.csv`'s
  md5 at generation time. `tables/v1_superseded/` holds the complete set of
  v1 tables (`table_main_final`, `h_sensitivity_kfixed_final`,
  `paired_diff_final`, `A1_clocks_final`, `A2_decile_final`,
  `A3_crosstab_final`, `A4_occupancy_final`, `B_trainpct_final`,
  `C_matched_final`, and their `.tex` siblings), unmodified, for audit.
- `figures/` — `fig_clocks_v2.pdf`, `fig_residual_deciles_v2.pdf`,
  `fig_failure_deciles_v2.pdf`, all produced by `scripts/figures_final.py`
  from the v2 data. `figures/v1_superseded/` holds the three v1 figures.
...
**Regenerate every v2 table and figure from the canonical file (fastest, no
GPU, no raw data needed):**
```
python3 scripts/aggregate.py       # writes tables/*_v2.{csv,tex} and related
python3 scripts/figures_final.py   # writes figures/*_v2.pdf, reads data/per_unit_clocks_final_v2.csv + tables/A2_decile_v2.csv
```

**Rebuild `data/per_sample_final_v2.csv` from the 20 checkpoints in `models/`**
...
2. `python3 scripts/build_canonical.py` — reads `models/*.pt` + `meta_*.json`,
   writes `data/per_sample_final_v2.csv` (+ `.md5`,
   `per_unit_clocks_final_v2.csv`, `trainpct_thresholds_final_v2.csv`). This
   ...
   models, at h ∈ {5, 7, 10} in the same pass.
...
- **MC-Dropout**: `n_samples=50` per window (`models/bayesian_lstm.py::predict_mc`);
  ...
  variance: `sigma_total = sqrt(mean(sigma_b^2) + Var(mu_b))` — **[v3 fix]**
  averaging the per-draw variances *before* the square root (was
  `sqrt(mean(sigma_b)^2 + Var(mu_b))`, which understates total variance by
  Jensen's inequality). Dropout draws are seeded deterministically per
  ...
  reproducible regardless of call order or process. (v1 never seeded this;
  see `legacy_leaked/` note — orthogonal to the v3 variance-formula fix.)
...
- **CUSUM change-point detection** — **[v3 redefinition]** baseline window is
  `max(floor(0.2*n), 20)` scores of the unit's OWN trajectory (was a
  hardcoded `scores[:10]`), scaling with unit length instead of a fixed
...
- **Mondrian CP — online mode**: one `MondirianCP(alpha=0.10, W_max=200,
  ...
  whose calibration scores are unusually tight).
- **Mondrian CP — split mode** — **[v3 redefinition, was "frozen"]**: NOT an
  online/adaptive instance. `build_mondrian_split_quantiles()` fills each of
  the 3 stage buckets ONCE from ALL `cal_units`' scores (uniform weight — no
  λ, no time index, no exponential decay), then takes the standard split-CP
  empirical quantile `ceil((m+1)(1-alpha))/m` (`method='higher'`) per stage.
  ...
  all). The previous "frozen" definition (`copy.deepcopy()` of the online
  ...
  calibration bucket is empty or its quantile level exceeds 1 (standard
  split-CP small-sample rule) — never from buffer/weight thinness, since
  there is no buffer. Per-engine ECR aggregation reports two numbers for
  Mondrian CP: one excluding unbounded samples ("finite-only"), and one
  treating every unbounded interval as covered ("unbounded-as-covered" —
  correct by definition, since an infinite-width interval covers any finite
  true value), plus the unbounded rate itself (numerator/denominator both
  given) — never one silently substituted for the other. This finite-only
...
- **Estimand and unique-engine-block bootstrap**: the headline ECR estimand
  ...
  Paired-difference confidence intervals (`paired_diff_v2.csv`) use a
...
1. **Sample-size subsampling attribution analysis** (`subsample_instability.py`
   ...
   principle. It has not been recomputed on seeded, v2 predictions and
...
2. **The early Stage 0-R2 "4/16" cohort-restriction result**, and the
   ...
   `tables/table_trainpct_full_vs_cohort.tex`, `tables/table_cusum_cohort_check.csv`):
   14/16 flips for the train-set-percentile cohort (matching an independent
   external recomputation exactly), 0/16 flips for the CUSUM cohort. The
   4/16 and 2/4 figures must not be cited; only the corrected 14/16 and 0/16
   figures are current.
...
│   ├── per_sample_final_v2.csv
│   ├── per_sample_final_v2.md5
│   ├── per_unit_clocks_final.csv          (v1, superseded)
│   ├── per_unit_clocks_final_v2.csv
│   ├── trainpct_thresholds_final.csv      (v1, superseded)
│   └── trainpct_thresholds_final_v2.csv
├── models/
...
├── tables/
│   ├── table_main_v2.{csv,tex}
│   ├── table_mondrian_unbounded_v2.csv
│   ├── table_B_trainpct_v2.csv
│   ├── table_trainpct_cohort.{csv,tex}
│   ├── table_trainpct_full_vs_cohort.{csv,tex}
│   ├── table_cusum_cohort_check.csv
│   ├── paired_diff_v2.csv
│   ├── table_paired_diff_v2.tex
│   ├── table_matched_v2.{csv,tex}
│   ├── bucket_geometry_v2.csv
│   ├── A1_clocks_v2.csv
│   ├── A2_decile_v2.csv
│   └── v1_superseded/
