...
HEADER_COMMENT = """\
# Stage 0-R3 canonical per-sample data source (per_sample_final_v3.csv).
...
def run():
    ...
    tmp_path = OUT / 'per_sample_final_v3_tmp.csv'
    final_path = OUT / 'per_sample_final_v3.csv'
    clock_path = OUT / 'per_unit_clocks_final_v3.csv'
    thresh_path = OUT / 'trainpct_thresholds_final_v3.csv'
    if tmp_path.exists():
        tmp_path.unlink()
    ...
    with open(OUT / 'per_sample_final_v3.md5', 'w') as f:
        f.write(md5 + '\n')
    return md5
