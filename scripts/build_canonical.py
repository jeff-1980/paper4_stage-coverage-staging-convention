...
def build_mondrian_split_quantiles(cal_series_list, h_mult, k_mult=K_FIXED, alpha=ALPHA):
    ...
    split-CP empirical quantile is computed per stage as the direct k-th order
    statistic, k=ceil((m+1)(1-alpha)) (1-indexed; q_hat=np.inf if k>m) --
    [Stage 0-R3 fix] previously this was approximated via
    np.quantile(scores, ceil((m+1)(1-alpha))/m, method='higher'), which is
    NOT exactly the k-th order statistic: np.quantile's 'higher' method scales
    its virtual index by (m-1), not m, so dividing k by m first and rescaling
    inside np.quantile silently shifts the selected index by one for
    generic m (e.g. m=397 selected the 360th smallest score for k=359, verified
    against sorted(scores)[358] vs the old np.quantile call). This REPLACES
    the previous 'frozen' definition (deepcopy of the online AdaptiveLambdaCP
    instance, predict-only) -- that was still an online/adaptive machine
    underneath, just not fed eval-time feedback; this is genuinely the
    classical, non-adaptive Mondrian split CP.
    Returns {stage: q_hat}, q_hat=np.inf if a stage's calibration bucket is
    empty or k exceeds m (standard split-CP small-sample rule)."""
    ...
    for stg in ['early', 'middle', 'late']:
        ...
        q[stg] = np.inf if k > m else float(np.sort(np.array(buckets[stg]))[k - 1])
    return q
...
HEADER_COMMENT = """\
# Stage 0-R4 canonical per-sample data source (per_sample_final_v4.csv).
...
# v4 vs v3 (per_sample_final_v4.csv, superseded but kept on disk): ONE fix,
# scoped to Split CP only, no retraining --
#   Split CP's single pooled calibration quantile q_hat is now the direct
#   k-th order statistic, k=ceil((nC+1)(1-alpha)) (1-indexed; q_hat=inf if
#   k>nC, which does not occur in practice) -- was approximated via
#   np.quantile(cal_scores_all, ceil((nC+1)(1-alpha))/nC, method='higher'),
#   the same off-by-one-prone approximation fixed for Mondrian CP (split) in
#   v3 (see build_mondrian_split_quantiles docstring below for why it's not
#   exactly the k-th order statistic). Only q_hat and
#   lo_sp/hi_sp/width_sp/covered_sp differ from v3; every other column,
#   including stage_cusum (computed from s_t, which does not depend on
#   q_hat), is bit-identical to v3, verified by diff.
# v3 vs v2 (per_sample_final_v2.csv, superseded but kept on disk): ONE fix,
# scoped to Mondrian CP (split) only, no retraining --
#   build_mondrian_split_quantiles()'s per-stage empirical quantile is now the
#   direct k-th order statistic, k=ceil((m+1)(1-alpha)) (1-indexed; q_hat=inf
#   if k>m) -- was approximated via np.quantile(scores, ceil((m+1)(1-alpha))/m,
#   method='higher'), which is NOT exactly the k-th order statistic (its
#   virtual index is scaled by (m-1), not m, so dividing k by m first shifts
#   the selected index by one for generic m). Only the mf/covered_mf/is_inf_mf
#   columns (and their _h5/_h10 siblings) differ from v2; mc/sp/mo columns and
#   all stage_* labels are bit-identical to v2, verified by diff.
...
def process_fd_seed(fd, seed, device):
    ...
    # [Stage 0-R4 fix] direct k-th order statistic, matching the Mondrian CP
    # (split) fix in build_mondrian_split_quantiles -- was
    # np.quantile(cal_scores_all, ceil((nC+1)(1-alpha))/nC, method='higher'),
    # which is NOT exactly the k-th order statistic (see that function's
    # docstring for why). k>nC would mean q_hat=inf, but does not occur in
    # practice since nC (calibration pool size) is far larger than 1/alpha.
    kC = int(np.ceil((nC + 1) * (1 - ALPHA)))
    q_hat = np.inf if kC > nC else float(np.sort(cal_scores_all)[kC - 1])
...
def run():
    ...
    tmp_path = OUT / 'per_sample_final_v4_tmp.csv'
    final_path = OUT / 'per_sample_final_v4.csv'
    clock_path = OUT / 'per_unit_clocks_final_v4.csv'
    thresh_path = OUT / 'trainpct_thresholds_final_v4.csv'
    if tmp_path.exists():
        tmp_path.unlink()
    ...
    with open(OUT / 'per_sample_final_v4.md5', 'w') as f:
        f.write(md5 + '\n')
    return md5
