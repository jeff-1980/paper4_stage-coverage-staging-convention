"""Redraws all 3 v2 figures: data/per_unit_clocks_final_v2.csv (from
build_canonical.py) and tables/A2_decile_v2.csv (from aggregate.py, run that
first)."""
import pandas as pd
import matplotlib
matplotlib.use('Agg')
...
def fig_clocks():
    clocks = pd.read_csv(DATA / 'per_unit_clocks_final_v2.csv')
    ...
    fig.tight_layout()
    fig.savefig(FIGURES / 'fig_clocks_v2.pdf')
    fig.savefig(FIGURES / 'fig_clocks_v2.png', dpi=150)
    plt.close(fig)
    print('wrote fig_clocks_v2.pdf')


def fig_residual_deciles():
    df = pd.read_csv(TABLES / 'A2_decile_v2.csv')
    ...
    fig.tight_layout()
    fig.savefig(FIGURES / 'fig_residual_deciles_v2.pdf')
    fig.savefig(FIGURES / 'fig_residual_deciles_v2.png', dpi=150)
    plt.close(fig)
    print('wrote fig_residual_deciles_v2.pdf')
...
def fig_failure_deciles():
    ...
    fig.tight_layout()
    fig.savefig(FIGURES / 'fig_failure_deciles_v2.pdf')
    fig.savefig(FIGURES / 'fig_failure_deciles_v2.png', dpi=150)
    plt.close(fig)
    print('wrote fig_failure_deciles_v2.pdf')
