"""Redraws all 3 figures from the canonical source: data/per_unit_clocks_final.csv
(from build_canonical.py) and tables/A2_decile_final.csv (from aggregate.py).
...
def fig_clocks():
    ...
    fig.tight_layout()
    fig.savefig(FIGURES / 'fig_clocks_final.pdf')
    fig.savefig(FIGURES / 'fig_clocks_final.png', dpi=150)
    plt.close(fig)
    print('wrote fig_clocks_final.pdf')


def fig_residual_deciles():
    df = pd.read_csv(TABLES / 'A2_decile_final.csv')
    ...
    fig.tight_layout()
    fig.savefig(FIGURES / 'fig_residual_deciles_final.pdf')
    fig.savefig(FIGURES / 'fig_residual_deciles_final.png', dpi=150)
    plt.close(fig)
    print('wrote fig_residual_deciles_final.pdf')
...
def fig_failure_deciles():
    ...
    fig.tight_layout()
    fig.savefig(FIGURES / 'fig_failure_deciles_final.pdf')
    fig.savefig(FIGURES / 'fig_failure_deciles_final.png', dpi=150)
    plt.close(fig)
    print('wrote fig_failure_deciles_final.pdf')
